  ...
  setTimeout(() => {
    ...
    if (!peer.destroyed && peer.disconnected) {
      try {
        ...
        console.warn('Falha ao reconectar peer:', err);
      }
    }
  }, delayMs);
}
...
let myProfile = {
  name: localStorage.getItem('profile_name') || (isHost ? 'Mozão da Bella 🐺💙' : 'Neném da Thaya'),
...
let remoteProfile = {
  name: isHost ? 'Neném da Thaya' : 'Mozão da Bella 🐺💙',
